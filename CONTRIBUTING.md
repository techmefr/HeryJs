...
- Run the full gate before opening the PR: `pnpm lint`, `pnpm run arch:check`, `pnpm run lint:capabilities`, `pnpm run lint:dev-guard`, `pnpm test`, `pnpm run build`. CI runs the same checks, but catching it locally first is faster for everyone.
