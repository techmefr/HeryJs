...
import type {
  StorageDriver,
  StoredObject,
} from '#technical/storage/storage-driver';
 ...
 * instructions tell developers to inject the driver and pass keys straight in,
...
// The module's zero-config default driver: files live on local disk, "signed
// URLs" are served by StorageController with an HMAC signature + expiry, the
// same trick already used for the signal SSE token (no header, so the
// signature travels in the query string). Declare another driver in
// hery.config.ts under storage.drivers for anything beyond local dev.
@Injectable()
