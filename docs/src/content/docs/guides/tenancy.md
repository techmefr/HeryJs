...
Deciding how a new tenant comes into being — from a subdomain, an invitation token, an admin-provisioned account — is a product decision, the same way team roles and invitations are (see [Teams](./teams.md)). HeryJs gives you the boundary and enforces it on every query; it does not guess your provisioning story. Running real multi-tenancy means replacing or gating `AuthController.register` (`src/technical/auth/auth.controller.ts`) yourself, so a new signup is assigned the tenant it actually belongs to instead of inheriting the schema default.
