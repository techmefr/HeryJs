...
`GET /expose` is what a form-rendering admin page reads: each entry already carries everything needed to draw a field — its kind, its bounds, its default. It answers "what can _you_ run", not "what exists": each action's capability is resolved against the caller's own subject, and one that denies them leaves the action out of the catalog entirely. An action's name and parameter schema describe an operation — a model to prune, a retention window to shorten — so the same check that gates running it gates seeing it. `POST /expose/:action` resolves the action, checks its environment filter, checks its capability against `subjectOf(user)`, then resolves each declared param: an input value if the caller sent one, its spec's `default` otherwise, validated either way against a Zod schema derived from the spec (`schemaFor`, in `exposition-validation.ts`) — so the bounds are declared once and checked the same way regardless of who calls it. One audit entry is written before the action runs, with `model: 'exposition'`, `operation: <actionName>`, and every resolved param by name.
